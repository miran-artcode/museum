/* ============================================================
   유물 발상 단계 (3차시 학습지): 데이터 정의와 스타일

   관찰한 사회 문제·현상을 그 장소에 실제로 있는 유물(사물)에 드러난 사용 흔적으로
   시각화하는 발상 활동이다. 일곱 단계로 나눈다.

     단계 1 관찰 방법 정하기  (2차시 끝, 교실에서 고름)   s3o
     단계 2 장면 모으기      (한 주 과제)                s3a
     단계 3 문제 정하기      (3차시 수업)                s3p → 값은 s3a.problem·contact·why
     단계 4 안 보이는 것 찾기                            s3t1
     단계 5 물건 고르기                                  s3t2
     단계 6 흔적 정하기                                  s3t3
     단계 7 짝의 해석 확인과 고쳐 쓰기                  s3t4
     돌아보기 (단계가 아님): 태도·가까운 방법            s3c (+ s3e.mode·why)

   단계 사이 논리: 방법(1) → 그 방법으로 본 장면(2) → 장면이 가리키는 문제(3)
   → 문제에서 안 보이는 것과 그 이유(4) → 이유가 정해 준 곳에서 몸이 닿는 물건(5)
   → 물건에 남는 자국과 그것을 만든 반복(6) → 남이 읽는지 확인하고 고쳐 마지막 문장(7).

   저장 키는 2026-08 설계(아홉 단계)의 것을 그대로 쓴다. 이미 쌓인 학생 기록이
   깨지지 않게 하기 위해서다. 없앤 칸(s3e.sub 등)은 화면에서만 사라지고 저장은 남는다.
   필드에 `key`가 있으면 그 키에 저장한다(단계 3 카드가 s3a.* 에 쓰는 장치).

   2026-09 개정: 단계 9 → 7. 옛 T 코드 대응: T1→T1, T2→T2, T3→T3, T4(드러내는 방법)→돌아보기,
...
export const ATTITUDE_HINTS = {
  "고발": "잘못을 드러낸다",
  "경고": "앞으로 올 일을 알린다",
  "공감": "그 사람의 입장이 되어 보게 한다",
  "기록": "사라질 것을 기록한다",
  "질문": "보는 사람이 스스로 묻게 한다",
};

/* 단계 1·3 카드 위에 한 번씩 뜨는 「유물 발상 단계란」 상자의 본문 */
export const LADDER_INTRO = "관찰한 사회 문제나 사회 현상을, 그 장소에 실제로 있는 유물(사물)에 드러난 사용 흔적으로 시각화하는 활동입니다. 이 흔적이 4차시에 설계할 유물의 출발점이 됩니다. 예: 새벽 배송 노동의 시간 → 손수레 오른쪽 손잡이만 닳고 테이프를 감은 자국. 사진에 사람은 넣지 않고, 눈물·사슬 같은 상징도 쓰지 않습니다. 사회 문제는 나쁜 점만이 아니라 우리 사회의 모습과 현상을 두루 뜻합니다.";

/* 일곱 단계. keys는 그 단계에 속한 저장 키. 대시보드·연구 자료 내보내기가 이 표를 그대로 쓴다.
   학생 화면은 key(T1~T7)를 찍지 않고 n·name만 쓴다. */
export const TRANSLATE_STAGES = [
  { key: "T1", n: 1, hw: true, session: "2차시", sec: "s3o", name: "관찰 방법 정하기", full: "관찰 방법을 정한다",
    keys: ["s3o.method", "s3o.methodWhy"],
    ask: "한 주 동안 우리 동네를 어떤 방법으로 관찰할지 정합니다.",
    out: "방법 하나, 고른 이유 한 줄",
    look: "이유가 보려는 것의 성격과 이어지는지" },
  { key: "T2", n: 2, hw: true, session: "2차시", sec: "s3a", name: "장면 모으기", full: "장면을 모은다",
    keys: ["s3a.scenes"],
    ask: "그 방법대로 다니며 눈에 띈 장면을 적습니다.",
    out: "장면 세 줄(언제·어디서·무엇을)",
    look: "해석이 아니라 본 것을 적었는지" },
  { key: "T3", n: 3, session: "3차시", sec: "s3p", name: "문제 정하기", full: "문제를 정한다",
    keys: ["s3a.problem", "s3a.contact", "s3a.why"],
    ask: "단계 2에 기록한 세 장면이 공통으로 보여 주는 사회 문제나 사회 현상은 무엇이고, 나는 그것을 어디서 직접 만나는지 적습니다.",
    out: "문제 한 마디, 만나는 때와 곳, 뺀 문제와 비교한 이유",
    look: "만나는 때와 곳이 실제 동선에 있는지" },
  { key: "T4", n: 4, session: "3차시", sec: "s3t1", name: "안 보이는 것 찾기", full: "안 보이는 것과 이유를 찾는다",
    keys: ["s3t1.list", "s3t1.pick", "s3t1.pickWhy"],
    ask: "이 문제에서 사진에 찍히지 않는 것은 무엇이고, 왜 보이지 않는지 적습니다.",
    out: "안 보이는 것 2~3개와 이유, 고른 하나, 이유 한 줄",
    look: "이유가 여러 개로 나뉘는지, 고른 이유에 자국 가능성이 나오는지" },
  { key: "T5", n: 5, session: "3차시", sec: "s3t2", name: "물건 고르기", full: "물건을 고른다",
    keys: ["s3t2.cands", "s3t2.dropWhy"],
    ask: "그 장소에 실제로 있는 물건 가운데 사람 몸이 닿는 것을 고릅니다.",
    out: "물건 2~3개(어디에·누가 어떻게), 고른 하나, 뺀 이유",
    look: "누가 어떻게 만지는지 적었는지, 뺀 이유가 있는지" },
  { key: "T6", n: 6, session: "3차시", sec: "s3t3", name: "흔적 정하기", full: "흔적으로 표현한다",
    keys: ["s3t3.traces", "s3t3.stmt1"],
    ask: "그 물건 어느 위치에 어떤 자국이 남고, 어떤 동작을 얼마나 반복해야 생기는지 적습니다.",
    out: "흔적 네 칸, 흔적 문장(첫 번째)",
    look: "위치·자국이 사진에 찍히는 말인지, 반복에 횟수와 기간이 있는지" },
  { key: "T7", n: 7, session: "3차시", sec: "s3t4", name: "짝의 해석 확인과 고쳐 쓰기", full: "짝의 해석을 확인하고 고쳐 마지막 문장을 쓴다",
    keys: ["s3t4.reverse", "s3t4.stmt2"],
    ask: "자국만 듣고 다른 사람이 내 문제를 알아차리는지 확인하고, 알아차리지 못하면 어느 단계로 돌아가 고칠지 정합니다.",
    out: "짝이 읽은 것, 맞은 정도, 돌아간 단계와 고친 것, 마지막 문장",
    look: "다르게 읽혔을 때 돌아가 고쳤는지, 추상적인 말이 사진에 찍히는 말로 바뀌었는지" },
];

/* 돌아보기는 단계가 아니라 일곱 단계 밖의 카드. 연구 자료는 이 네 키를 따로 읽는다 */
export const REFLECT_KEYS = ["s3c.attitude", "s3c.attWhy", "s3e.mode", "s3e.why"];

/* 돌아보기(단계 6·7)에서 자동 요약(s3b.*)을 다시 계산하게 하는 저장 키 */
export const DERIVE_KEYS = ["s3t1.list", "s3t1.pick", "s3t2.cands", "s3t3.traces"];

/* 옛 기록의 「되돌아간 단계」 문자열 → 새 코드. 옛 T4(드러내는 방법)는 가장 가까운 앞 단계(단계 3)으로 집계한다 */
...
/* ---------- 학습지 카드 ----------
   tag: 학생 화면 카드 머리에 찍는 이름(「단계 3」). code는 교사 화면·CSV 전용.
   hw: 한 주 과제 표시. intro: 「유물 발상 단계란」 상자. summary: 「내 발상 한눈에」 상자.
   필드의 max는 글자 수 상한, def는 예시(placeholder). key는 저장 키 재지정. */

export const LADDER_HW_SECTIONS = [
  {
    id: "s3o", session: "2차시", code: "B1", tag: "단계 1", hw: true, ladder: true, intro: true, title: "관찰 방법 정하기",
    note: "지금 교실에서, 한 주 동안 우리 동네를 어떤 방법으로 관찰할지 하나 고릅니다. 방법마다 잘 보이는 것과 놓치는 것이 다릅니다. 고른 방법대로 본 것은 다음 카드(단계 2)에 적습니다.",
    fields: [
      { k: "method", t: "cards", src: "obs", label: "이번 한 주 동안 쓸 관찰 방법 (하나를 고릅니다)" },
      { k: "methodWhy", t: "text", max: 120, label: "이 방법을 고른 이유 한 줄", def: "새벽에 오가는 사람을 볼 수 없으니, 내가 다니는 길을 그려 놓고 상자가 놓이는 곳부터 표시해 보려고" },
      { k: "blind", t: "text", max: 120, opt: true, label: "이 방법으로는 놓칠 것 같은 것 하나 (선택)", def: "새벽에 오는 사람은 내 동선에 없어서 지도에 안 잡힌다" },
    ],
  },
  {
    id: "s3a", session: "2차시", code: "B2", tag: "단계 2", hw: true, ladder: true, title: "장면 모으기",
    note: "고른 방법대로 다니며 눈에 띈 장면을 세 개 적습니다. 느낌이 아니라 본 것만 씁니다. 문제를 정하는 일은 다음 수업에서 합니다.",
    fields: [
      {
        k: "scenes", t: "scenes", label: "본 것 세 가지: 언제 · 어디서 · 무엇을",
        steps: ["위 「이렇게 합니다」 세 순서를 그대로 따릅니다", "‘힘들어 보였다’가 아니라 ‘손잡이에 테이프가 감겨 있었다’처럼, 눈에 보인 것만 적습니다"],
      },
      { k: "photos", t: "images", opt: true, mm: true, label: "그 장소 사진 (있으면 · 최대 3장 · 사람 얼굴이 나오지 않게)" },
      { k: "sound", t: "audio", max: 30, opt: true, mm: true, label: "그 장소 소리 (있으면 · 30초 · 사람 목소리가 담기지 않게)" },
    ],
  },
];
...
export const LADDER_CLASS_SECTIONS = [
  {
    id: "s3p", session: "3차시", code: "B3", tag: "단계 3", ladder: true, intro: true, title: "문제 정하기",
    note: "단계 2에 기록한 세 장면(언제·어디서·무엇을 보았는지)을 다시 읽고, 그 장면들이 공통으로 보여 주는 사회 문제나 사회 현상을 한 마디로 적습니다. 그리고 그 문제를 내가 어디서 직접 만나는지 씁니다. 뉴스에 나오는 큰 문제보다 내가 실제로 지나는 곳에서 만나는 문제가 좋습니다.",
    fields: [
      { k: "problem", key: "s3a.problem", t: "text", max: 40, label: "내 문제 (한 마디로)", def: "새벽 배송 노동" },
      { k: "contact", key: "s3a.contact", t: "text", max: 80, label: "이 문제를 내가 직접 만나는 때와 곳 (장면에 나온 장소를 적습니다. 직접 본 것이 없으면 문제를 바꿉니다)", def: "밤 11시 아파트 분리수거장, 그리고 매일 아침 우리 집 현관 앞" },
      { k: "why", key: "s3a.why", t: "text", max: 120, label: "떠올렸다가 뺀 다른 문제 하나와, 그 대신 이 문제를 고른 이유 한 줄", def: "교실 분리수거도 떠올렸지만 그건 다 보이는 일이라 뺐다. 이 일은 내 집 앞에서 벌어지는데 내가 자는 시간에 일어난다" },
    ],
  },
  {
    id: "s3t1", session: "3차시", code: "B4", tag: "단계 4", ladder: true, title: "안 보이는 것 찾기",
    note: "이 문제에서 사진에 찍히지 않는 것을 두세 가지 적고, 왜 보이지 않는지 이유를 하나씩 고릅니다. 고른 이유가 다음 단계에서 물건을 찾을 곳을 정해 줍니다.",
    fields: [
      {
        k: "list", t: "invis", label: "사진에 찍히지 않는 것과, 안 보이는 이유",
        steps: ["장면 속에 있었지만 사진에는 안 찍히는 것: 여러 번 쌓여야 보이는 사람의 시간, 벽·문 뒤의 일, 세지 않은 숫자, 이미 없어진 것, 늘 그래서 아무도 문제로 안 치는 것", "이유 버튼을 누르면 그 이유에 맞는 ‘찾을 곳’이 아래에 표시됩니다. 이유가 서로 다르면 더 좋습니다"],
      },
      { k: "pick", t: "pickinv", label: "이 가운데, 다음 단계에서 고를 물건이 자국으로 대신 말해 줄 것 하나" },
      { k: "pickWhy", t: "text", max: 100, label: "고른 이유 한 줄 (그것은 물건에 자국으로 남을 수 있나요?)", def: "상자 수는 숫자라 자국이 없지만, 몸과 시간은 손잡이에 닳음으로 남는다" },
    ],
  },
  {
    id: "s3t2", session: "3차시", code: "B5", tag: "단계 5", ladder: true, title: "물건 고르기",
    note: "그 장소에 실제로 있는 물건을 두세 개 적고, 어디에 있고 누가 어떻게 만지는지 씁니다. 몸이 닿지 않는 물건에는 자국이 남지 않습니다.",
    fields: [
      {
        k: "cands", t: "cands", label: "물건 후보와, 어디에 있고 누가 어떻게 만지는지",
        steps: ["머릿속 물건이 아니라, 단계 2의 장면에 실제로 나온 물건이나 단계 3의 ‘만나는 곳’에 놓여 있는 물건을 적습니다", "‘누가 어떻게 만지는지’를 쓸 수 없는 물건은 지우지 말고 「제외」를 누릅니다. 고른 물건 하나에 「선택」을 누르면 그 물건이 단계 6으로 갑니다"],
      },
      { k: "dropWhy", t: "text", max: 100, label: "뺀 물건 하나와 뺀 이유 한 줄", def: "종이 상자: 며칠이면 버려져서 몇 년치 자국이 쌓이지 않는다" },
    ],
  },
  {
    id: "s3t3", session: "3차시", code: "B6", tag: "단계 6", ladder: true, title: "흔적 정하기",
    note: "고른 물건의 어느 위치에 어떤 자국이 남는지, 그 자국이 생기려면 어떤 동작을 얼마나 반복해야 하는지 씁니다. 사진에 찍히는 것만 씁니다.",
    fields: [
      {
        k: "traces", t: "traces", label: "흔적 네 칸 (위치 · 자국 · 동작 · 반복)",
        steps: ["위치는 한 군데만. 손잡이 전체가 아니라 손바닥이 닿는 쪽", "마음·의미·상징(눈물·사슬)은 쓰지 않습니다. 사진으로 찍으면 무엇이 찍히는지만 씁니다"],
      },
      { k: "stmt1", t: "stmt", stage: "T6", label: "흔적 문장 (첫 번째)" },
    ],
  },
  {
    id: "s3t4", session: "3차시", code: "B7", tag: "단계 7", ladder: true, title: "짝의 해석 확인과 고쳐 쓰기",
    note: "문제 이름은 말하지 말고 물건과 흔적만 짝에게 읽어 준 뒤, 짝이 무엇으로 이해했는지 그대로 적습니다. 다르게 이해했다면 돌아갈 단계를 정해 위 단계 표시줄에서 그 단계를 눌러 고친 뒤, 여기로 돌아와 마지막 문장을 씁니다.",
    fields: [
      {
        k: "reverse", t: "reverse", label: "짝 한 명에게 읽어 주고 해석 듣기",
        steps: ["짝에게는 위 「읽어 줄 것」만 읽어 줍니다. 문제 이름과 ‘노동’ 같은 큰 말은 말하지 않습니다", "다르게 이해했다고 해서 실패가 아닙니다. 어디로 돌아가 무엇을 고쳤는지가 가장 중요한 기록입니다"],
      },
      { k: "stmt2", t: "stmt", stage: "T7", prev: "s3t3.stmt1", label: "흔적 문장 (마지막)" },
    ],
  },
  {
    id: "s3c", session: "3차시", code: "D", tag: "돌아보기", summary: true, title: "발상 단계를 마치며: 태도와 가까운 방법",
    note: "일곱 단계가 끝났습니다. 이 자국으로 보는 사람에게 무엇을 하려는지, 그리고 내 작업이 오늘 감상한 「사회를 드러내는 여덟 가지 방법」 중 어디에 가장 가까운지 정리해 둡니다. 4차시 캡션의 말투가 여기서 정해집니다.",
    fields: [
      { k: "attitude", t: "select", opts: ATTITUDES, hints: ATTITUDE_HINTS, label: "이 자국으로 나는 보는 사람에게 무엇을 하려고 하는지 (하나)" },
      { k: "attWhy", t: "text", max: 100, label: "그렇게 정한 이유 한 줄", def: "고발보다 먼저, 이 일이 있었다는 사실을 기록하고 싶다" },
      { k: "mode", key: "s3e.mode", t: "cards", src: "engage", label: "오늘 감상한 사회를 드러내는 여덟 가지 방법 중 내 작업과 가장 가까운 것 하나" },
      { k: "modeWhy", key: "s3e.why", t: "text", max: 120, label: "그 방법이 내 문제와 맞는 이유 한 줄", def: "그 사람을 만날 수 없으니, 그 사람이 쓴 물건이 대신 말하게 하고 싶어서" },
