...
export const LADDER_INTRO = "눈에 안 보이는 사회문제를, 그 자리에 실제로 있는 물건에 남은 자국으로 옮겨 적는 일입니다. 예: 새벽 배송 노동의 시간 → 손수레 오른쪽 손잡이만 닳고 테이프를 감은 자국. 나중에 만들 사진에 사람은 넣지 않고, 눈물·사슬 같은 상징도 쓰지 않습니다.";
...
export const LADDER_HW_SECTIONS = [
  {
    ...
    note: "지금 교실에서, 한 주 동안 우리 동네를 어떤 방법으로 볼지 하나 고릅니다. 방법마다 잘 보이는 것과 놓치는 것이 다릅니다. 고른 방법대로 본 것은 다음 카드(계단 2)에 적습니다.",
    fields: [
...
export const LADDER_CLASS_SECTIONS = [
  {
    ...
    note: "모아 온 장면 셋이 가리키는 사회문제를 한 마디로 적고, 그 문제를 내가 어디서 직접 만나는지 씁니다. 뉴스에 나오는 큰 문제보다 내가 실제로 지나는 자리에서 만나는 문제가 좋습니다.",
    fields: [
    ...
    fields: [
      {
        ...
        steps: ["장면 속에 있었지만 사진에는 안 찍히는 것: 여러 번 쌓여야 보이는 사람의 시간, 벽·문 뒤의 일, 세지 않은 숫자, 이미 없어진 것, 늘 그래서 아무도 문제로 안 치는 것", "까닭 버튼을 누르면 그 까닭에 맞는 ‘찾을 곳’이 아래에 뜹니다. 까닭이 서로 다르면 더 좋습니다"],
      },
      { k: "pick", t: "pickinv", label: "이 가운데, 다음 계단에서 고를 물건이 자국으로 대신 말해 줄 것 하나" },
    ...
    fields: [
      {
        ...
        steps: ["머릿속 물건이 아니라, 계단 2의 장면에 실제로 나온 물건이나 계단 3의 ‘만나는 곳’에 놓여 있는 물건을 적습니다", "‘누가 어떻게 만지나’를 못 쓰는 물건은 지우지 말고 「뺌」을 누릅니다. 남길 물건 하나에 「고름」을 누르면 그 물건이 계단 6으로 갑니다"],
      },
  ...
  },
  {
    ...
    fields: [
      {
        k: "traces", t: "traces", label: "흔적 네 칸 (자리 · 자국 · 동작 · 되풀이)",
  ...
  },
  {
    ...
    note: "문제 이름은 말하지 말고 물건과 흔적만 짝에게 읽어 준 뒤, 짝이 무엇으로 읽었는지 그대로 적습니다. 다르게 읽혔다면 돌아갈 계단을 정해 위 계단 표시줄에서 그 계단을 눌러 고친 뒤, 여기로 돌아와 마지막 문장을 씁니다.",
    fields: [
      {
  ...
  },
  {
    ...
    note: "번역이 끝났습니다. 이 자국으로 보는 사람에게 무엇을 하려는지, 그리고 내 작업이 오늘 감상한 「사회를 드러내는 여덟 가지 방법」 중 어디에 가장 가까운지 짚어 둡니다. 4차시 캡션의 말투가 여기서 정해집니다.",
    fields: [
