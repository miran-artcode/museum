...
/* ============================================================
   ...
   ------------------------------------------------------------
   왜 바꾸었나
     처음 설계는 물음 아래에 「고르기 → 찾기 → 정리」 세 계단을 미리 놓았다.
     계단 안에는 답의 보기(선택·명명·전시 / 빛·초점·질감…)까지 들어 있었다.
     학생은 계단을 그대로 실행했고, 그 결과 답이 모두 세 문장·같은 순서·
     같은 낱말로 닮아 갔다. 발판이 사고의 순서와 답의 형태까지 정해 준 탓이다.

   무엇으로 바꾸었나
     ① 먼저 쓴다 — 물음만 보고 쓴다. 틀려도 된다. 보기는 없다.
        (문제를 먼저 붙들게 한 뒤 도움을 주는 편이 배움이 깊다는 ‘생산적 실패’의 순서)
     ② 첫 답을 굳힌다 — 그 순간의 글이 첫 답으로 따로 남는다.
     ③ 되묻는다 — 답의 내용을 주지 않고 학생 자신의 문장을 찌르는 물음이 온다.
        물음별 되묻기(그 물음에만 맞는 것)와 공통 되묻기(개념·설계에 두루 쓰는 것)를
        학번+물음으로 정한 순서로 배정하므로 옆 사람과 다른 되묻기를 받는다.
...
export const INQ_GENERIC = {
  "개념": [
    "네가 쓴 답이 틀리는 경우를 하나 만들어 보면, 답은 어떻게 바뀌는가",
    "답 속에서 가장 힘이 센 낱말 하나를 고르고, 그 말을 다른 말로 바꾸면 무엇이 달라지는가",
    "이 답은 오늘 본 것 없이도 쓸 수 있던 답인가 — 아니라면 어느 지점이 오늘 본 것에서 왔는가",
    "누가 이 답에 가장 반대할 것 같은가. 그 사람은 어디를 찌를까",
    "이 답이 맞다면 그다음에 무엇이 따라 나오는가 — 따라 나오는 것 가운데 받아들이기 어려운 것이 있는가",
    "‘항상’인가 ‘대체로’인가 ‘가끔’인가 — 답의 범위를 한 단계 좁히거나 넓히면 무엇이 달라지는가",
    "네 답을 한 사람에게 말로 설명한다면 어디에서 ‘예를 들면’이 필요한가",
  ],
  "설계": [
    "그 선택 때문에 관람자가 못 보게 되는 것은 무엇인가",
    "네가 정한 것을 반대로 하면 무엇이 무너지는가 — 무너지지 않는다면 왜 그렇게 정했는가",
    "이 결정을 실제로 하려면 무엇이 더 필요한가 — 시간·자리·재료·허락 가운데 무엇이 걸리는가",
    "이 결정을 다음 차시의 나에게 넘긴다면, 그 나는 무엇을 물어볼까",
    "관람자 대신 다른 작가가 이 결정을 본다면 무엇을 먼저 지적할까",
    "네 답에 ‘왜’와 ‘어떻게’ 가운데 모자란 쪽은 어느 쪽인가",
  ],
};

/* ---------- 물음 줄기 — 스스로 물음을 쓸 때 참고하는 뼈대 (내용 없음) ---------- */
export const INQ_STEMS = [
  "…이 아닌 경우는 없는가",
  "…와 …는 어떻게 다른가",
  "…이 맞다면 그다음엔 무엇이 따라오는가",
  "누구의 자리에서 보면 달라지는가",
  "…를 뒤집으면 무엇이 무너지는가",
  "내 답에서 가장 약한 곳은 어디인가",
...
export const INQUIRY_SECTIONS = {
  q1: inqSec(1, [
    concept("작가가 손대지 않은 사물이 작품이 되려면 그 전에 무슨 일이 있어야 하는가. 그 일은 꼭 작가가 해야 하는가", [
      "네가 말한 ‘그 일’이 교탁 위 청소용 밀대에 일어난다면 밀대는 작품이 되는가. 안 된다면 네 답에 무엇이 빠진 것인가",
      "그 일이 일어났는데도 작품이 되지 못한 사물을 하나 떠올릴 수 있는가",
      "그 일을 작가가 아닌 사람(관람자·미술관·우연)이 해도 되는가. 된다면 ‘작가’는 무엇을 하는 사람인가",
      "「샘」이 전시에서 거부당했던 시점에 그것은 작품이었는가 — 네 답의 기준으로 판정하면",
      "오늘 본 작품 가운데 네 답으로 설명하기 가장 어려운 것은 무엇인가",
    ]),
    design("오늘 본 작품 하나를 우리 교실에 들여온다면 어디에 어떻게 놓겠는가 — 그리고 그 자리에서 그것은 여전히 작품인가", [
      "아침에 교실에 들어온 사람이 그것을 무엇으로 볼까. 그 오해는 네 계획에 들어 있는 것인가",
      "높이를 30cm 낮추거나 벽 하나를 옮기면 무엇이 달라지는가 — 달라지는 것이 없다면 자리를 정한 이유는 무엇인가",
      "작품 캡션이나 안내문 없이 두겠는가. 두겠다면 무엇이 그것을 작품으로 붙들어 주는가",
      "교실의 어떤 물건이 그 작품과 헷갈릴 수 있는가. 헷갈리게 두겠는가, 막겠는가",
      "일주일 뒤에도 그 자리가 맞을까 — 청소·수업·이동 가운데 무엇이 계획을 흔드는가",
    ]),
    debate("미술인지 아닌지를 정하는 권한은 작가·미술계·관람자에게 각각 얼마씩 있는가"),
  ]),
  q2: inqSec(2, [
    concept("작품 캡션의 어떤 정보가 이미지의 읽기를 가장 크게 바꾸는가. 그 정보를 지우면 이미지에는 무엇이 남는가", [
      "네가 고른 정보가 거짓이라는 것을 관람자가 알게 되면 읽기는 원래대로 돌아가는가",
      "같은 정보를 캡션이 아니라 옆 사람의 말로 들었다면 힘이 같았을까 — 다르다면 힘을 준 것은 무엇인가",
      "A와 B 가운데 네 눈이 더 오래 머문 쪽은 어디였고, 그 시간은 정보 때문이었나 문장 때문이었나",
      "캡션의 정보 가운데 읽기를 거의 바꾸지 못한 것도 있는가. 왜 그것은 힘이 없었는가",
      "네 답이 한 가지 정보로 끝난다면, 그 정보가 없는 유물 사진은 읽을 수 없는 것인가",
    ]),
    design("내가 만들 유물의 작품 캡션에서 관람자가 첫 줄로 읽었으면 하는 정보는 무엇인가. 그 줄을 읽은 관람자가 이미지에서 무엇을 찾기 시작하기를 바라는가", [
      "첫 줄에 놓은 정보를 맨 아래로 내리면 관람자의 눈길은 어디로 가는가",
      "그 첫 줄은 허구 고지보다 먼저 읽혀야 하는가, 나중이어야 하는가",
      "관람자가 그 첫 줄을 믿지 않는다면 나머지 캡션은 무엇을 할 수 있는가",
  ...
  q3: inqSec(3, [
    concept("사물의 흔적이 사람에 대해 말할 수 있는 것과 끝내 말할 수 없는 것은 무엇인가", [
      "‘말할 수 없다’고 쓴 것을, 흔적 두 개를 겹치면 말할 수 있게 되는가",
      "흔적을 읽는 사람이 바뀌면(가족·경찰·고고학자) 흔적이 말하는 내용도 바뀌는가 — 그렇다면 말하는 쪽은 흔적인가 읽는 사람인가",
      "흔적이 사람 대신 말하게 두는 것이 그 사람에게 더 공정한가, 덜 공정한가",
      "「르완다 프로젝트」의 눈은 흔적인가 얼굴인가 — 네 답의 기준으로 판정하면",
      "흔적이 거짓말을 할 수 있는가. 있다면 ‘말할 수 있는 것’에 무엇을 덧붙여야 하는가",
    ]),
    design("내가 고른 문제에서 카메라가 찍을 수 있는 것과 찍지 못하는 것은 무엇이며, 찍지 못하는 것 가운데 내 유물이 떠맡을 것은 무엇인가", [
      "‘찍지 못한다’고 쓴 것이 정말 못 찍는 것인가, 찍으면 안 되는 것인가, 찍어도 안 보이는 것인가",
      "찍을 수 있는 것만으로 이 문제를 전달한다면 무엇이 빠지는가 — 그 빠짐이 곧 내 유물의 자리인가",
      "내 유물이 떠맡은 것을 관람자는 유물의 어느 자리에서 읽어 낼 수 있는가",
      "찍을 수 없는 것을 유물로 옮기면서 내가 덧붙인 짐작은 무엇인가",
      "당사자가 네 나눔을 본다면 어느 줄에 고개를 저을까",
    ]),
    debate("당사자가 아닌 사람이 그 문제를 작품으로 다루는 일은 어디까지 정당한가"),
  ]),
  q4: inqSec(4, [
    concept("흔적이 그 자리에 생기려면 어떤 사용이 얼마나 반복되어야 하는가", [
      "그 횟수를 절반으로 줄이면 흔적은 어떻게 달라지는가 — 옅어지는가, 자리가 바뀌는가, 아예 안 생기는가",
      "같은 흔적이 다른 이유로도 생길 수 있는가. 있다면 관람자가 네 이유로 읽게 하려면 무엇이 더 있어야 하는가",
      "네가 짐작한 사용자가 왼손잡이였다면, 키가 20cm 작았다면 흔적은 어디로 옮겨 가는가",
    ...
    design("내 유물에서 관람자가 가장 먼저 볼 곳은 어디이며, 그렇게 만들려면 화면을 어떻게 짜야 하는가", [
      ...
      "그곳으로 시선을 모으는 장치가 유물의 ‘기록 사진’답지 않게 보일 위험은 없는가",
      "가장 먼저 볼 곳과 가장 오래 볼 곳은 같은 곳인가",
      "박물관 기록 사진은 보통 시선을 모으지 않는다 — 네 화면은 그 관례를 따를 것인가, 어길 것인가",
  ...
  q5: inqSec(5, [
    concept("사진처럼 보이게 만드는 요소 가운데 우리 눈이 가장 먼저 믿는 것은 무엇인가", [
      ...
      "네가 먼저 믿은 것이 사진의 성질인가, 카메라의 버릇인가",
      "그 요소는 진짜 사진에도 항상 있는가 — 그것이 없는 진짜 사진을 하나 떠올릴 수 있는가",
      "눈이 먼저 믿는 것과 나중에 의심하는 것이 같은 요소일 수 있는가",
      "이 답을 내 회차 화면 전부에 대 보면 예외가 있는가",
    ]),
    design("내가 고른 한 점과 버린 것들의 차이는 어디에 있는가 — 그 차이를 말로 다 옮기고 나면 그래도 남는 것이 있는가", [
      "버린 것 가운데 하나를 오늘 다시 본다면 같은 판단을 할까. 아니라면 무엇이 그때의 판단을 만들었는가",
      "차이를 설명한 말이 전부 화면에서 확인되는가 — 확인 안 되는 말이 있다면 그것은 무엇을 가리키는가",
  ...
  q6: inqSec(6, [
    concept("낡음을 늘렸는데 오히려 의심이 커지는 화면에는 무엇이 빠져 있는가", [
      "네가 ‘빠졌다’고 쓴 것을 넣으면 의심이 정말 사라지는가 — 아니면 다른 의심이 생기는가",
      "낡음을 줄였는데 오히려 믿음이 커지는 경우도 있는가. 있다면 네 답은 어떻게 되는가",
      "진짜 유물 가운데 ‘너무 낡아서 의심스러운’ 것을 본 적이 있는가",
      "의심하는 쪽은 눈인가, 머리인가 — 네 답은 어느 쪽을 설득하려는 것인가",
      "내 화면에 그 ‘빠진 것’이 있는가. 없다면 넣을 것인가, 빠진 채로 둘 것인가",
    ]),
    design("내 화면에서 고치지 않기로 한 부분은 무엇이며 왜 그대로 두는가", [
      "고치지 않은 부분을 관람자가 결함으로 본다면 그것은 네 결정을 무너뜨리는가",
      "‘고치면 무엇이 사라지는가’에 답이 없다면 그것은 결정인가, 미룸인가",
  ...
  q7: inqSec(7, [
    concept("같은 작품을 교실 벽과 미술관 벽에 걸었을 때 무엇이 달라지는가", [
      ...
      "‘벽’을 ‘관람자’로 바꿔 읽어도 네 답은 성립하는가",
    ]),
    design("내 작품 옆에 놓일 다른 작품 한 점을 내가 고른다면 무엇이며 왜인가", [
      "그 작품을 반대쪽 벽으로 옮기면 무엇이 사라지는가",
      "옆에 놓인 뒤 네 작품이 더 잘 읽히는가, 그 작품이 더 잘 읽히는가 — 둘 다가 아니라면 왜 고른 것인가",
  ...
  q8: inqSec(8, [
    concept("이미지·명칭·작품 캡션·진열 가운데 이 학급의 작품들이 의미를 만든 방법은 어느 쪽에 몰려 있는가", [
      ...
      "네가 고른 작품과 반대 증거가 되는 작품은 없는가 — 있다면 경향은 다시 어떻게 말해야 하는가",
      "네 작품은 그 경향 안에 있는가, 밖에 있는가",
      "네 가지 가운데 우리 반이 거의 쓰지 않은 방법이 있다면, 쓰기 어려워서인가 필요 없어서인가",
      "1차시의 다섯 후보(기술·선택·명명·전시·관념)로 같은 물음을 다시 묻는다면 답이 달라지는가",
    ]),
    design("관람자가 내 의도와 다르게 읽었을 때 나는 무엇을 고치고 무엇을 그대로 두겠는가", [
      ...
      "고치겠다고 한 것을 고치면 원래 의도가 그대로 남는가, 의도도 조금 바뀌는가",
...
/* ---------- 배정 — 학번+물음으로 순서를 정해 옆 사람과 다른 되묻기를 받게 한다 ---------- */
function hash32(s) {
  let h = 2166136261;
...
function SelfQ({ value, onChange, level }) {
  ...
  return (
    <div className="inq-selfq">
      <label className="inq-l">내가 나에게 묻는 물음 하나 — 위 답에서 내가 아직 답하지 않은 것</label>
      <input value={value || ""} maxLength={200} placeholder="예를 들어 ‘…이 아닌 경우는?’처럼, 내 답을 흔들 물음"
        onChange={(e) => onChange(e.target.value)} />
      <div className="inq-stems">
        <button type="button" className="inq-link" onClick={() => setStems(!stems)}>{stems ? "물음 줄기 접기" : "물음 줄기 보기"}</button>
        ...
        <div className="inq-foot">
          {text.trim().length >= MIN_V1 ? (
            <button type="button" className="btn small" onClick={freeze}>
              {level === "self" ? "첫 답 굳히기 → 스스로 되묻기" : "첫 답 굳히기 → 되묻기 받기"}
            </button>
          ) : null}
          <span className="hint">
            {!text.trim() ? "먼저 씁니다. 다 쓰면 「첫 답 굳히기」로 넘어갑니다."
              : text.trim().length < MIN_V1 ? "조금 더 쓰면 첫 답을 굳힐 수 있습니다."
              : "굳히면 지금 글이 첫 답으로 남고, 그 뒤에 고친 것과 나란히 기록됩니다."}
          </span>
        </div>
      ) : (
  ...
  return (
    <div className="inq-trace">
      <div className="inq-tl">
        ...
        {tr.v1Level ? "수준 " + levelLabel(tr.v1Level) : "굳히기 전"}
        {route ? " · 출발점 " + route.label : ""}
        {tr.v1 ? " · 첫 답 " + tr.v1.trim().length + "자 → 지금 " + fin.trim().length + "자 · 겹침 " + sim + "%" : ""}
        {gap != null && gap > 0 ? " · 굳힌 뒤 " + (gap >= 60 ? Math.round(gap / 60) + "분" : gap + "초") + " 더 고침" : ""}
      </div>
      ...
      {filled(tr.selfQ) && <div className="inq-tv"><b>스스로 쓴 물음</b>{tr.selfQ}</div>}
    </div>
  );
}
    ...
    <div className="card">
      ...
      <div className="card-body">
        <p style={{ fontSize: 13, marginBottom: 12 }}>
          탐구 질문은 <b>먼저 쓰고 → 첫 답을 굳히고 → 되묻기를 받아 → 같은 칸에서 고치는</b> 순서로 진행됩니다.
          기본값은 앞 차시에서 앱이 많이 묻고 뒤로 갈수록 학생이 스스로 묻도록 줄어드는 일정입니다.
          학급의 형편에 따라 차시별로 바꿀 수 있습니다. 이미 첫 답을 굳힌 학생의 기록은 그때의 수준이 그대로 남습니다.
        </p>
