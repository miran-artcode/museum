...
const ROWS = [
  ...
  },
  {
    k: "출처",
    a: "수습 경위 기록 없음",
    b: "물류 창고 하역장 터의 지층에서 수습. 같은 형태가 여러 점 나왔고 모두 오른쪽만 닳아 있음",
  },
];

const READ_A = "낡은 쇠 토막 하나. 어디가 닳았는지는 굳이 들여다보지 않게 된다.";
const READ_B = "오른쪽 그립이 벗겨진 부분이 먼저 눈에 들어온다. 그 부분을 한 손으로 오래 쥐고 끌던 사람이 떠오른다.";
        ...
        <div className="lcmp-body">
          ...
          {!compact && (
            <div className="lcmp-how">
              <b>이 자료로 답하는 순서</b>
              <ol>
                <li>캡션 B를 읽고 사진을 다시 봅니다. 캡션 A만 읽었을 때는 보이지 않다가 새로 보이게 된 것을 적어 둡니다.</li>
                <li>캡션 B의 다섯 줄(제목 · 연대 · 재질 · 용도 · 출처) 가운데 한 줄을 지웠다고 가정하고 사진을 봅니다. 1에서 적은 것이 그대로 보이는지, 사라지는지 줄마다 확인합니다.</li>
                <li>지웠을 때 1에서 적은 것이 가장 많이 사라지는 줄이 「이미지를 읽는 방식을 가장 크게 바꾼 정보」입니다. 답에는 그 줄의 이름, 그 줄이 사진에서 무엇을 보게 했는지, 그 줄을 지우면 사진에 무엇이 남는지를 씁니다.</li>
              </ol>
              <div className="lcmp-demo">
                답에 들어갈 세 가지: 고른 줄의 이름 · 그 줄이 사진에서 무엇을 보게 했는지 · 그 줄을 지우면 사진에 무엇이 남는지. 어느 줄을 고를지는 사진을 직접 보고 정합니다.
              </div>
            </div>
          )}
        </div>
