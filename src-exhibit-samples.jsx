...
export const EXHIBIT_SAMPLES = [
  {
    ...
    context:
      "물류 창고 하역장 자리로 추정되는 지층에서 파이프가 절단된 상태로 수습되었다. 함께 나온 파편 가운데 오른쪽 그립만 고무가 벗겨져 금속이 드러나 있고, 왼쪽 그립은 표면이 온전하다. 벗겨진 자리에는 직물 테이프를 여러 겹 감아 수리한 흔적이 남아 있으며, 테이프 아래 금속면은 손의 압력이 닿는 폭만큼 광택이 났다. 같은 형태의 도구가 다수 발굴되었으나 모두 오른쪽에만 마모가 집중되어 있다.",
    coll: "○○고등학교 2학년 ○반 가상 컬렉션 / 작가 ○○○",
    aiScope: "이미지 생성 및 부분 수정, 화면 편집. 명제표 문안과 작가 노트는 학생이 작성함.",
    notice: "이 이미지는 생성형 AI로 제작한, 실재한 적 없는 유물입니다.",
    problem: "새벽 배송 노동",
    attitude: "기록",
    display: "단독 진열",
    note:
      "우리 집 문 앞에는 매일 새벽 상자가 놓인다. 나는 그것을 놓고 가는 사람을 한 번도 본 적이 없다. 얼굴을 모르는 사람의 노동이 매일 우리 집 현관에 도착한다는 사실이 이상하게 느껴졌다.\n그 사람을 그리려다 그만두었다. 본 적이 없는 얼굴을 상상해서 그리면 내가 만드는 것은 내 짐작이 된다. 대신 그 사람이 쥐었을 물건을 만들기로 했다. 배송 기사들이 쓰는 손수레 영상과 사진을 찾아보니 손잡이가 한쪽만 심하게 닳아 있는 경우가 많았다. 무거운 짐을 실은 손수레를 한 손으로 기울여 끌기 때문이라고 한다.\n그래서 오른쪽만 닳게 만들었다. 벗겨진 고무 아래 금속이 드러나고, 그 위에 테이프를 감아 계속 쓰다가, 테이프도 닳아 다시 금속이 보이는 자리까지 화면에 넣었다. 이 물건을 300년 뒤에 발굴한 사람은 이 시대에 한 손으로 무언가를 오래 끌던 사람들이 있었다는 것을 알게 될 것이다. 나는 그 사람을 지금도 보지 못한다.",
  },
  {
    id: "ex-b",
    no: "예시 B",
  ...
  },
  {
    ...
    context:
      "목욕 시설로 추정되는 건물의 탈의 공간 자리에서 여러 개가 흩어진 채 수습되었다. 번호표에 새겨진 숫자는 1번부터 60번까지 확인되며, 그 가운데 30번대 번호표의 표면 마모가 가장 심하다. 출입구에서 가까운 자리의 신발장이 먼저 사용되었기 때문으로 보인다. 열쇠와 번호표를 잇던 고무줄은 대부분 삭아 끊어졌고, 번호표 가장자리에는 손톱으로 반복해 긁힌 자국이 남아 있다. 손이 자주 닿은 면은 새겨진 숫자의 획이 얕아져 읽기 어려운 상태다.",
    coll: "○○고등학교 2학년 ○반 가상 컬렉션 / 작가 ○○○",
    aiScope: "이미지 생성 및 부분 수정, 화면 편집. 명제표 문안과 작가 노트는 학생이 작성함.",
    notice: "이 이미지는 생성형 AI로 제작한, 실재한 적 없는 유물입니다.",
    problem: "사라지는 동네",
    ...
    display: "파편 진열",
    note:
      "우리 동네 목욕탕이 작년에 문을 닫았다. 어릴 때 할머니 손을 잡고 가던 곳이다. 지금은 셔터가 내려가 있고 앞에 부동산 스티커가 붙어 있다.\n그 안에 있던 물건 중에 신발장 열쇠가 생각났다. 들어갈 때 받아서 손목에 차고 있다가 나올 때 돌려주는 물건. 수십 년 동안 매일 수백 명의 손을 거쳤을 텐데, 문을 닫으면 아무도 찾지 않는 물건이 된다.\n번호를 37번으로 정한 이유는 출입구에서 가까운 자리가 먼저 닳는다는 이야기를 목욕탕을 하시던 분께 들었기 때문이다. 그래서 30번대가 가장 닳은 것으로 설정하고, 숫자 획이 얕아져 잘 안 보이도록 했다. 화면에서는 숫자가 결국 읽히지 않는다. 명제표에만 37번이라고 적혀 있다. 관람자가 화면을 믿을지 명제표를 믿을지 잠깐 망설이는 그 자리가 이 작품에서 내가 만들고 싶었던 것이다.",
  },
];
...
.exs-tail{margin:22px auto 0;border-top:1px dashed var(--line);max-width:620px;padding-top:10px;text-align:center;color:var(--sub);font-size:12px}
      ...
      <div className="exs-head">
        ...
        <p>
          ...
          관람 방식은 학생 작품과 같습니다 — 1차에는 명제표를 가리고 보고, 2차에는 명제표를 펼쳐 봅니다.
        </p>
      </div>
