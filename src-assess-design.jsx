...
const COL = {
  ...
  judgements: {
    ...
    valid: "현재 명단의 배정표와 자리·쌍·좌우·반복까지 대조해 받아들인 판정이면 1. 옛 명단·중복·변조·자기 작품 포함은 0",
    screen_i: "화면 순번(0부터)",
  ...
  scores: {
    ...
    n: "순위 분모(비교된 적 있는 작품 수)",
    pct: "실제 백분위 = (n − rank) / (n − 1)",
      ...
      <div className="card-body">
        ...
        <div ref={bodyRef} key={sweep.n}>
          ...
          <Sec id="1" title="연구 질문과 연구의 위치" open={isOpen(true)}>
            ...
            <ol className="ad-ol">
              <li><b>신뢰도·타당도.</b> 고등학생 동료 쌍대비교 척도가 학급 안에서 SSR ≥ {QUALITY_MIN.ssrAdopt}에 이르는가. 교사의 가린 채 평정(2인 또는 교사의 부분 쌍대비교)과 r ≈ .6 수준으로 수렴하는가 <Ref>Li 외 2016; Jones &amp; Alcock 2014; 초·중등 메타분석 r = .68: Sanchez 외 2017</Ref>. 창의성 6축 행동지표와는 어떻게 상관하는가.</li>
          ...
          <Sec id="2" title="흐름과 자료 구조: 단계마다 무엇이 어디에 남는가" open={isOpen(false)}>
            ...
            <T head={["단계", "학생이 하는 것", "저장 위치", "잠금·규칙"]} rows={[
              ["제출", "기록지의 최종 작품(번호·제목·작품 캡션·aiLevel·대표 이미지)을 확인하고 제출 확정. 비교 중에 원본이 바뀌어도 판정자마다 같은 작품을 보도록 스냅샷으로 굳힘", <M key="a">submissions/{"{학번}"}</M>, "확정 시 잠김. 명단에 든 작품은 교사도 풀지 않음"],
              ["자기평가 ①", "5문항 1~5점 + 근거 한 문장 + 학급 안 등수 예측", <M key="b">assess/{"{학번}"}.self.s1</M>, "제출 시 잠김"],
              ["동료 비교", "배정된 k쌍 + 숨은 역순 반복 1쌍. 쌍마다 승자·이유·결정적 축 태그·표시(어려움·작성자 짐작)", <M key="c">assess/{"{학번}"}.judge.p1</M>, "쌍 단위 저장, 되돌리기 없음. 명단을 다시 확정하면 judge.r_<해시>에 새로 쌓이고 옛 블록은 남음"],
              ["자기평가 ②", "①을 보지 않은 채 같은 5문항·근거·등수 예측에 답해 「현재 점수 확정」(lockedAt). 그다음 화면에서 ①과 나란히 보고 변화 사유", <M key="d">assess/{"{학번}"}.self.s2</M>, "확정 시 점수·예측이 굳음. 변화 사유 전에 끊겨도 확정값은 자료"],
              ["결과", "받은 이유 문장·태그 먼저, 자리(밴드)는 「자리 보기」를 눌러야", <M key="e">assess/{"{학번}"}.result</M>, "교사만 씀(보안 규칙). 학생은 자기 것만 읽음"],
              ["명단·배정표", "교사가 「비교 명단 확정」할 때 한 번 굳힘: fixedAt·seed·k·repeat·works·judges·plan·hash·stats", <M key="f">meta/peerRoster</M>, "교사만 씀, 로그인하면 읽음. 소유자 정보 없음"],
              ["설정", "단계 스위치와 k·반복·최소 시간·이유 하한·확신도·공개 범위·종합 질문", <M key="g">config.peer</M>, "k·반복은 명단 확정 순간 배정표에 굳음"],
            ]} />
            <p>명단을 문서 하나로 굳히는 것이 이 설계의 가장 중요한 한 줄입니다. 명단이 실시간으로 바뀌면 학생마다 다른 쌍을 보게 되어 노출 균형이 깨지고 점수를 견줄 수 없습니다 <Ref>research-design §7: 학생 기기에서 즉석 난수를 쓰지 않는다</Ref>. 집계는 학생 문서의 판정을 배정표와 자리·쌍·좌우·반복까지 대조해 맞지 않는 것을 버립니다. 학생이 자기 문서에 쓸 수 있는 구조에서 자기 작품을 끼워 넣은 판정이 점수를 움직이면 안 되기 때문입니다.</p>
          </Sec>
          ...
          <Sec id="3" title="확정 파라미터: 값과 근거" open={isOpen(false)}>
            ...
            <T head={["항목", "값", "어디에", "근거"]} rows={[
              ...
              ["배정", "비적응·균형 고정, seed로 결정론, 명단 확정 시 문서로 굳힘", <M key="r">meta/peerRoster</M>, "Bramley 2015; Bramley & Vitello 2019; Crompvoets 외 2020"],
              ...
              ["자기평가", SELF_ITEMS.length + "문항(종합 + 4축) + 근거 + 등수 예측. ②는 ①을 보지 않고 답해 굳힌 뒤 나란히 봄", <M key="se">SELF_ITEMS</M>, "Schraw 2009; Nederhand 외 2019(앵커링 회피)"],
          ...
          <Sec id="4" title="배정 규칙: 작품마다 정확히 2k회" open={isOpen(false)}>
            <p>서버 없이도 학급 전체의 노출이 고르게 되도록, 명단의 작품 수를 n, 판정자 색인을 j(작품 없는 판정자는 명단 뒤의 가상 색인), 회차를 r = 1…k라 할 때 판정자 j의 r회차 쌍은 다음과 같습니다.</p>
            <F>{"A = (j + r) mod n\nB = (j + r + d_r) mod n"}</F>
            ...
            <p>n = 4~40, k = 10·12 × 20 seed, 그리고 n = 4·k = 3과 n = 30·k = 4 × 200 seed 회귀 검사(<M>npm test</M>)에서 예외 없이 해가 있고 그래프가 이어집니다. 쌍 목록은 (명단, 학번, 회차)로 매번 계산되므로 앱을 껐다 켜도 같은 순서가 나옵니다.</p>
          ...
          <Sec id="5" title="측정 도구: 화면·문항·기록 필드" open={isOpen(false)}>
            ...
            <p>종합 질문 하나만 묻습니다(교사가 카드 1에서 고칠 수 있음). 축을 쌍마다 여러 번 물으면 판정 시간이 늘고 피로가 신뢰도를 깎으며, 판정 중에 축을 나열하면 총체적 판단이 분석적으로 바뀝니다 <Ref>Leech &amp; Chambers 2022; Jones &amp; Wheadon 2015</Ref>.</p>
            <F>{PEER_QUESTION}</F>
            <ul className="ad-ul">
              ...
              <li>「판단이 어려웠다」(경험된 복잡성 <Ref>van Daal 외 2017</Ref>)와 「누구 작품인지 알 것 같다」(익명성 민감도 분석용)를 늘 둡니다. 확신도 5단계({CONF_LABELS[0]} … {CONF_LABELS[4]})는 교사가 켤 때만 묻습니다.</li>
            ...
            <ul className="ad-ul">
              <li><b>근거 문항(필수):</b> 이번 점수를 정할 때 가장 중요하게 본 자기 작품의 구체적 특징이나 제시자료의 근거 한 가지.</li>
              <li><b>등수 예측:</b> 「우리 반 n점 가운데 내 작품은 몇 번째쯤일까」 슬라이더. 저장은 등수와 백분위 둘 다.</li>
              <li><b>②의 절차:</b> ①의 답·학급 결과·순위를 보여 주지 않은 채 답하게 하고 「현재 점수 확정」으로 굳힌 뒤, 그다음 화면에서 ①과 나란히 놓고 변화 사유를 받습니다. 「다시·향상」처럼 점수가 달라져야 한다는 암시를 쓰지 않고, 비교 활동 때문이라고 가정하지 말라고 안내합니다 <Ref>앵커링 회피: Schraw 2009; Nederhand 외 2019</Ref>.</li>
              <li><b>변화 사유:</b> {CHANGE_CODES.map((c) => "「" + c.label + "」").join(" · ")}. 「기억나지 않음」을 뺀 보기에서만 {MIN_WHY_CHANGE}자 이상의 짧은 서술을 받습니다.</li>
            </ul>
            ...
            <T head={["필드", "뜻"]} rows={[
              ...
              [<M key="7">conf</M>, "확신도 1~5(설정이 켜졌을 때만)"],
            ...
            <ul className="ad-ul">
              ...
              <li><b>앵커 4쌍</b>(src-anchor.jsx): 학급 밖 작품 네 쌍에 같은 이미지·다른 「AI 활용 범위」 라벨을 판정자마다 무작위로 붙여 고지 감점의 인과 효과를 잽니다(anchorScore 0~4). 본 판정 뒤에 두고, 연습 쌍·황금 통제 항목 역할과 겹치지 않게 합니다. 끝나면 같은 차시에 교사가 직접 밝힙니다(디브리핑).</li>
          ...
          <Sec id="7" title="신뢰도와 품질 판정 도구: 무엇을 어떤 기준으로 보는가" open={isOpen(false)}>
            ...
            <T head={["지표", "계산", "기준", "근거"]} rows={[
              ["SSR(척도분리신뢰도)", "(관측 분산(θ) − 평균 SE²) / 관측 분산(θ). Rasch 분리 신뢰도와 같은 식", "채택 ≥ " + QUALITY_MIN.ssrAdopt + " · 주의 " + QUALITY_MIN.ssr + "~" + QUALITY_MIN.ssrAdopt + " · 보류 < " + QUALITY_MIN.ssr, "Verhavert 외 2018(판정자 간 일치도로 읽을 것); Kinnear 외 2025(SSR이 반분보다 체계적으로 높으므로 .80을 권고, 그때 SHR > .70인 자료가 약 9할)"],
              ...
              ["배정 밖 판정 제외", "학생 문서의 항목을 배정표와 자리·쌍·좌우·반복·자기 작품까지 대조", "1건이라도 있으면 보류 사유로 표시(누가 그랬는지 함께)", "학생이 자기 문서에 쓸 수 있는 구조의 방어"],
            ...
            <ul className="ad-ul">
              ...
              <li><b>주의 사유</b>(공개는 되지만 학생 결과 화면의 「자리의 폭이 넓다」 문구가 켜짐): SSR {QUALITY_MIN.ssr}~{QUALITY_MIN.ssrAdopt} · 반분 중앙값 &lt; {QUALITY_MIN.splitHalf} 또는 미산출 · 「누구 작품인지 알 것 같다」 ≥ 20%.</li>
          ...
          <Sec id="8" title="보정 지표와 분석 계획" open={isOpen(false)}>
            ...
            <F>{"bias   = 예측 백분위 − 실제(BT) 백분위      (+면 과대평가)\n|bias| = 절대 정확도\ncalib  = |bias①| − |bias②|                 (+면 보정됨)   ← 주 결과 변수"}</F>
            <T head={["변수", "계산", "뜻"]} rows={[
              ...
              [<M key="4">peerScore</M>, "브래들리–테리 θ·pct·band", "동료 판단에 의한 작품의 위치"],
            ...
            <ol className="ad-ol">
              <li><b>학급별 기술.</b> SSR, 반분(보정 전·후 중앙값·IQR), 판정 수, 작품당 노출, θ 분산, |z| &gt; 2 판정 비율, infit 분포, 먼저 놓인 쪽 선택률(축별), 반복 일치율, 「판단이 어려웠다」 비율, 캡션 펼침률, 배정 밖 판정 제외 건수. 학급의 이미지·문장 동질화 지표(「연구」 탭)를 신뢰도와 나란히 보고합니다. 생성형 AI로 만든 작품은 학급 전체의 다양성이 줄 수 있고 <Ref>Doshi &amp; Hauser 2024; Zhou &amp; Lee 2024</Ref> 진점수 분산이 줄면 판정 품질과 무관하게 SSR이 낮아지기 때문입니다.</li>
              ...
              <li><b>보정.</b> calib을 주 결과로 ANCOVA(학급 더미), 양측. θ 삼분위와의 상호작용은 사전 지정 하나만. 사분위 막대 대신 연속 회귀(선형+이차)와 Glejser 이분산 검정 <Ref>Gignac &amp; Zajenkowski 2020</Ref>. 중등 학생은 과소평가하는 경우도 있어 <Ref>Nederhand 외 2020</Ref> 양측 검정. 기준(BT 백분위) 자체가 신뢰도 .80 안팎의 학급 내 척도이므로 ①→② 변화에는 평균 회귀와 기준의 불신뢰가 섞입니다. 기준 신뢰도를 함께 보고하고 해석을 제한합니다 <Ref>Brown, Andrade &amp; Chen 2015</Ref>. n ≈ 140·학급 5~6개에서는 탐색적입니다.</li>
              ...
              <li><b>판정 단위 결과</b>(판정 시간·「판단이 어려웠다」·척도와의 일치)는 판정 ⊂ 판정자 ⊂ 학급의 다층 구조로 분석합니다. 앵커 쌍의 검정력은 판정자 무작위 절편을 넣은 혼합 로지스틱을 simr로 모의해 산출합니다 <Ref>Green &amp; MacLeod 2016</Ref>(d ≈ 0.24를 잡을 검정력으로 설계).</li>
          ...
          <Sec id="9" title="CSV 변수 사전: 카드 5의 다섯 파일" open={isOpen(false)}>
            ...
            <p>이 명단의 판정은 배정표 대조를 거친 것(valid = 1)만 점수에 쓰이고, 옛 명단·대조 탈락 항목은 valid = 0으로 함께 내보내 재현할 수 있게 합니다. 숨은 반복은 rep_of가 채워진 행이며 점수 계산에서 빠집니다.</p>
            ...
            <p className="ad-h">판정자(적합도·위치·시간) · {H.current.judges.length}열</p>
          ...
          <Sec id="10" title="윤리: 익명·동의·정책·디브리핑" open={isOpen(false)}>
            <ul className="ad-ul">
              <li><b>익명성의 한계를 솔직하게.</b> 화면에는 작품 번호만 나오지만 submissions 문서 id가 학번이고 이미지가 media/{"{학번}"}_…에 있어 개발자 도구를 열면 소유자를 알 수 있습니다. <b>표시상 익명·저장상 준익명</b>이며 완전 익명은 서버에서 소유자를 지운 사본을 만들어야 가능합니다(후속 과제). 피평가자 이름 노출은 국내 K-12 동료평가 점수를 부풀리고 평가자 익명은 별 영향이 없었으며 <Ref>안부영 2014</Ref>, 익명은 더 비판적인 피드백과 안전감을 주고 11학년에게 비계로 작동했습니다 <Ref>Panadero &amp; Alqassab 2019; Rotsaert 외 2018</Ref>. 「누구 작품인지 알 것 같다」 표시율을 사전 등록하고 그 판정을 뺀 민감도 분석을 보고합니다.</li>
              <li><b>동의.</b> 기존 「연구」 탭의 6항목 분리 동의(개인정보 보호법 제22조 제1항)를 그대로 씁니다. 판정·자기평가는 수업 활동이므로 전원이 하되, 논문용 집계·CSV에서는 동의하지 않은 학생을 뺍니다. 동의 거부가 수행평가 점수에 영향 없음을 명시합니다. 학급은 무작위 코드(C1~C6)로 가명처리해 제28조의2(과학적 연구 목적 가명정보 처리)로 정당화합니다.</li>
          ...
          <Sec id="11" title="한계와 열린 문제" open={isOpen(false)}>
            <ul className="ad-ul">
              <li>청소년이 AI 생성 이미지·허구 캡션을 쌍대비교로 판정한 선행연구가 없습니다. 기대 SSR .80~.85(작품당 {twoK}회)는 계획치이며, .80에 못 미친 학급은 기술적 분석으로 돌립니다. 작품당 24회에서 SSR 점추정은 .80 근처에 걸리기 쉬우므로 <Ref>Crompvoets 외 2021</Ref> 시간이 허락하면 k = 15(작품당 30회, 약 24분)를 검토합니다.</li>
              ...
              <li>반복 쌍은 옆자리 학생이 알아챌 수 있으므로 「숨김」을 기대하지 않고 기술적 지표로만 씁니다. 앵커 쌍은 연습 쌍·황금 통제 항목 역할과 겹쳐 맡길 수 없습니다.</li>
          ...
          {/* ------------------------------------------------ 12 */}
          <Sec id="12" title="핵심 근거 문헌(전체 144편은 쌍대비교_구현_근거.md)" open={isOpen(false)}>
